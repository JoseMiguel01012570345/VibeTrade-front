    ...
    <div className="container vt-page">
      <div className="vt-offerpage">
        <div className="vt-card vt-offerhero">
          <button className="vt-btn vt-offerhero-back" onClick={() => nav(-1)}>
            <ArrowLeft size={16} />
          </button>
          <img
            className="vt-offerhero-img"
            src={offer.imageUrl}
            alt={offer.title}
          />
          <div className="vt-offerhero-body">
            ...
            >
              <Link to={`/store/${store.id}`} className="vt-store-badge">
                ...
                {store.name}
                {store.verified ? (
                  <span className="vt-verified">Verificado</span>
                ) : (
                  <span className="vt-unverified">No verificado</span>
                )}
              </Link>
              <Trust
                ...
                onClick={() => {
                  ...
                  window.setTimeout(() => {
                    answer(
                      offer.id,
                      qaId,
                      "Podemos coordinar disponibilidad. Gracias por tu consulta.",
                    );
