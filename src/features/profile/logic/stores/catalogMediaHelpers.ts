import type { OwnerStoreFormValues, StoreBadge } from "@features/market/logic/store/marketStoreTypes";
import type { StoreCustomAttachment } from "@features/market/logic/storeCatalogTypes";
import type { ProductPhotoSlot } from "../../Dtos/productPhotoSlot";
import { isProtectedMediaUrl } from "@shared/services/media/mediaClient";
import { revokeObjectUrlIfNeeded } from "@shared/services/media/dataUrl";
...
export function inferCatalogMediaKindFromUrl(
  url: string,
): StoreCustomAttachment["kind"] {
  const u = url.trim();
  if (!u) return "other";
  const lower = u.toLowerCase();
  if (lower.startsWith("data:image/")) return "image";
  if (lower.startsWith("data:application/pdf") || lower.includes("application/pdf"))
    return "pdf";
...
export function serviceCatalogImagePhotoUrlsFromSlots(
  slots: ProductPhotoSlot[],
): string[] {
  return slots
    .filter((s) => {
