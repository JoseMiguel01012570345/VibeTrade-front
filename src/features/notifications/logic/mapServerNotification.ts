import type { ChatNotificationDto } from '@features/chat/Dtos/thread/chatApiTypes';import type { NotificationItem, NotificationRouteMeta } from '../Dtos/notificationItem'
import type { OfferNotificationKind } from '../Dtos/offerNotificationKind'

function mapOfferNotificationKind(k: string | null | undefined): OfferNotificationKind {
...
function parseRouteTramoMeta(metaJson: string | null | undefined): NotificationRouteMeta {
  ...
  try {
    ...
    if (Array.isArray(preselRaw) && preselRaw.every((x) => typeof x === 'string')) {
      preselStopIds = (preselRaw as string[]).map((s) => s.trim()).filter(Boolean)
...
export function mapServerNotification(n: ChatNotificationDto): NotificationItem {
  ...
  const kind = isOfferOnly ? mapOfferNotificationKind(n.kind) : ('chat_message' as const)
  return {
    id: n.id,
    kind,
    title: `${n.authorLabel} · confianza ${n.authorTrustScore}`,
