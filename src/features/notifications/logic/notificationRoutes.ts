...
export function notificationDeepLink(
  ...
  if (n.kind === 'route_sheet_presel' && n.threadId) {
    ...
    if (sheet) {
      ...
      const stops =
        n.preselStopIds?.length ? n.preselStopIds.join(',')
        : n.stopId?.trim() ? n.stopId.trim()
        : ''
  ...
  if (n.kind === 'route_ownership_granted' && n.threadId && n.routeSheetId?.trim()) {
    const q = new URLSearchParams({ rail: '1', sheet: n.routeSheetId.trim() })
    return `/chat/${encodeURIComponent(n.threadId)}?${q.toString()}`
  }
  ...
  if (
    (n.kind === 'offer_comment' || n.kind === 'offer_like' || n.kind === 'qa_comment_like') &&
    n.offerId
  ) {
