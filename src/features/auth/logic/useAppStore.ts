...
import { getOpenChatThreadIdFromLocation } from "@features/chat/logic/thread/getOpenChatThreadIdFromLocation"
import { getSessionToken, isSessionActiveInStorage, SESSION_ACTIVE_KEY } from "@shared/services/http/sessionToken"
import {
  postMeTrustAdjust,
...
type AppState = {
  /** SesiÃ³n iniciada (p. ej. tras verificar OTP). La barra de confianza solo aplica al usuario autenticado. */
  isSessionActive: boolean
  /** Usuario de prueba local; el resto del catÃ¡logo viene del API. */
  me: User
  /** Nombres mostrados para otros ids (bootstrap / Mocks del backend). */
  profileDisplayNames: Record<string, string>
  /** Avatares de otros usuarios (`/api/v1/media/â€¦`) indexados por user id (p. ej. comprador en chat). */
  profileAvatarUrls: Record<string, string>
  /** Confianza de usuario por id (API pÃºblico / sesiÃ³n); p. ej. integrantes del chat. */
  profileTrustScores: Record<string, number>
  /** Historial de cambios de confianza por usuario (esta sesiÃ³n / dispositivo). */
  ...
  /** Modal global de autenticaciÃ³n (login/registro). */
  ...
  markAllRead: () => void
  /** VacÃ­a la lista local y debe usarse tras marcar leÃ­das en servidor para que no reaparezcan al sincronizar. */
...
export const useAppStore = create<AppState>((set, get) => ({
  isSessionActive: isSessionActiveInStorage(),
  me: { ...guestMe },
  profileDisplayNames: {},
  profileAvatarUrls: {},
  profileTrustScores: {},
