import type { RouteSheet, RouteSheetCreatePayload, RouteTramoFormInput } from "@features/chat/Dtos/route-sheet/routeSheetTypes";
import {
  estimadoInstantMs,
  ROUTE_ESTIMADO_ISO_LOCAL_RE,
...
} from "@features/chat/Dtos/route-sheet/routeSheetValidationTypes";
...
function validateCoordPairInner(
  latRaw: string | undefined,
  lngRaw: string | undefined,
): string | undefined {
  const latS = norm(latRaw);
  ...
  if (latS === "" || lngS === "")
    return "Indica latitud y longitud, o deja ambas vacías";
  if (parseLat(latRaw) === null) return "Latitud inválida (entre -90 y 90)";
  if (parseLng(lngRaw) === null) return "Longitud inválida (entre -180 y 180)";
  return undefined;
}
 ...
 */
export function getRouteSheetFormErrors(
  p: RouteSheetCreatePayload,
  opts?: { routeLegPaymentCurrency?: string | null },
): RouteSheetFormErrors {
  ...
  if (merc.length < MERC_MIN) {
    e.mercanciasResumen =
      merc.length === 0
        ? "Completa este campo"
        : `Describe con al menos ${MERC_MIN} caracteres`;
  } else {
  ...
  paradas.forEach((raw, i) => {
    ...
    if (!t1 && !t2) {
      ...
      if (msRec !== null && msEnt !== null && msRec >= msEnt) {
        mergeTramo(e, i, {
    ...
    {
      const x = requiredText(
        raw.responsabilidadEmbalaje,
        BLOQUE_MIN,
        FIELD_MAX,
      );
...
export function hasRouteSheetFormErrors(e: RouteSheetFormErrors): boolean {
  ...
  return Object.values(e.tramos).some(
    (t) => t && Object.values(t).some(Boolean),
  );
}
...
export function normalizeRouteSheetParadas(
  ...
  return paradas
    .map((p) => ({
      ...
      transportInvitedStoreServiceId:
        norm(p.transportInvitedStoreServiceId) || undefined,
      transportInvitedServiceSummary:
        norm(p.transportInvitedServiceSummary) || undefined,
      monedaPago: norm(p.monedaPago) || undefined,
    }))
    .filter(
      (p) => p.origen.length >= PLACE_MIN && p.destino.length >= PLACE_MIN,
    );
}
...
export function validateRouteSheetPayload(
  p: RouteSheetCreatePayload,
): string[] {
