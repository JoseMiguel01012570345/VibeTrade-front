...
} from "@features/chat/Dtos/paymentCheckoutBreakdownTypes";
import {
...
export function buildPaymentCheckoutBreakdown(
  ...
  bucket.forEach((b) => {
    ...
    const processorFee = processorFeeMinorEstimate(b.subtotalMinor, currency);
...
export function formatMoneyMajorFromMinor(amountMinor: number, currency: string): string {
  try {
    return new Intl.NumberFormat("es-AR", {
      ...
      minimumFractionDigits: currencyMinorDecimals(currency),
      maximumFractionDigits: currencyMinorDecimals(currency),
    }).format(minorToMajor(amountMinor, currency));
  } catch {
    const m = currencyMinorDecimals(currency);
