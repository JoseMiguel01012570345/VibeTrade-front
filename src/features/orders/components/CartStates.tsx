...
export function CartEmptyState({ homeHref }: Readonly<{ homeHref: string }>) {
  return (
    <div className="mx-auto w-full max-w-[1140px] px-4 py-8 sm:py-12">
      ...
      <div className="vt-storefront-section-panel mt-6 flex flex-col items-center rounded-[24px] border px-6 py-10 text-center shadow-[0_14px_36px_rgba(33,37,41,0.05)]">
        <p className="max-w-md text-base text-slate-500">
          Añade productos desde el catálogo para empezar tu pedido.
        </p>
        <Link
          to={homeHref}
          className="vt-storefront-accent-btn mt-5 inline-flex h-12 items-center justify-center rounded-full px-7 text-sm font-bold text-white transition"
        >
          Explorar tienda
        </Link>
      </div>
