...
function phoneLineFromResolved(
...
export function RouteSheetTransportistaPhoneField({
  ...
  const selectedService = selectedSvcId
    ? services.find((x) => x.id === selectedSvcId)
    : undefined;
        ...
        {phoneLocked && v ? (
          <div className="rounded-xl border border-[var(--border)] bg-[color-mix(in_oklab,var(--bg)_40%,var(--surface))] px-3 py-2.5">
            <div className="font-semibold text-[var(--text)]">
              {lockedDisplayName?.trim() ||
                pickedLabel ||
                "Transportista confirmado"}
            </div>
            <div className="vt-muted mt-0.5 font-mono text-[12px] break-all">
              {v}
            </div>
            {svcSummary ? (
              <div className="mt-2 text-[12px] leading-snug">
                <span className="font-bold text-[var(--muted)]">
                  Servicio indicado en la hoja:{" "}
                </span>
                {svcSummary}
              </div>
            ) : null}
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-[var(--border)] bg-[color-mix(in_oklab,var(--bg)_40%,var(--surface))] px-3 py-2.5">
            <div className="min-w-0">
              <div className="font-semibold text-[var(--text)]">
                {pickedLabel ?? "Contacto en la hoja"}
              </div>
              <div className="vt-muted mt-0.5 font-mono text-[12px] break-all">
                {v}
              </div>
      ...
      {pickerOpen ? (
        <>
          <button
            ...
            className={cn(
              mapBackdropLayerAboveChatRail,
              "fixed inset-0 border-0 bg-black/45 z-[130]",
            )}
            aria-label="Cerrar"
          ...
          >
            <div className="flex items-start justify-between gap-2 border-b border-[var(--border)] px-6 py-4">
              <div>
                <h2
                  id={`ruta-tramo-${tramoIndex}-pick-svc-title`}
            ...
            <div
              className={cn(
                modalFormBody,
                "max-h-[min(72vh,560px)] overflow-y-auto px-6 pb-6 pt-4 mt-0",
              )}
            >
              {pickerBusy ? (
